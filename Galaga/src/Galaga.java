...
public class Galaga {
	...
	public static void main(String[] args) {
		...
		// Creates the player and its lives
		...
		Enemies enemies[] = level.nextLevel(0);
		...
		do {
			while (!gameover) {
				...
				if (lastAction.equals("right")) {
					...
					System.exit(0);
				} else if (lastAction.startsWith("new game") || newGame == true) {
					...
					level = new Level();
					enemies = level.nextLevel(levelCheat);
					levelCheat = 0;
					board.gb_setValueLevel(level.getCurrentLevel());
					for (int e = 0; e < enemies.length; e++) {
						board.gb_addSprite(enemies[e].getId(), enemies[e].getImage(), true);
				...
				if (!gameover) {
					...
					for (int e = 0; e < enemies.length; e++) {
						if (enemies[e].isVisible()) {
							if (!ghost && checkCollision(enemies[e], player)) {
								gameover = true;
							}
					...
					for (int t = 0; t < torpedoes.length; t++) {
						if (torpedoes[t].updatePosition()) {
							...
							// Else, the torpedo is from an enemy
								...
								if (!ghost && checkCollision(player, torpedoes[t])) {
									torpedoes[t].setVisible(false);
									board.gb_setSpriteVisible(torpedoes[t].getId(), false);
			...
			if (lastAction.equals("exit game")) {
				...
				String commands[] = lastAction.split(" ");
				
				if (commands.length >= 2) {
					switch (commands[1]) {
					case "level":
						levelCheat = Integer.parseInt(commands[2]);
						newGame = true;
						gameover = false;
						board.gb_setTextPlayerName("CHEATER");
						board.gb_println("Warped game to level " + levelCheat);
						break;
					case "ghost":
						newGame = true;
						ghost = true;
						gameover = false;
						board.gb_setTextPlayerName("CHEATER");
						board.gb_println("Ghost game");
						break;
					}
				}
			}

			// Sleep to make the board responsive
			try {
				Thread.sleep(Constants.GAME_DELAY);
			} catch (InterruptedException ie) {
				// Nothing to do
			}
		} while (true); // Infinite loop, until application is closed
	}
